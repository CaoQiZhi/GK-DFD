...
from dataset.cifar100 import (
    ...
    get_cifar100_dataloaders_balanced,
...
def parse_option():
    ...
    parser.add_argument('--path_t', type=str, required=True, help='teacher model snapshot')
    ...
    # GK-DFD
    parser.add_argument('--gkdfd_classes_per_batch', default=8, type=int,
                        help='number of classes in each balanced batch')
    parser.add_argument('--gkdfd_cac_scale', default=0.5, type=float,
                        help='CAC attenuation for cross-class graph edges')
    parser.add_argument('--gkdfd_pca_ratio', default=2.0, type=float,
    ...
    parser.add_argument('--gkdfd_kd_weight', default=0.0, type=float,
                        help='optional KL loss weight in addition to the paper objective')

    opt = parser.parse_args()

    if opt.learning_rate is None:
        if opt.distill == 'gkdfd':
            opt.learning_rate = 0.02 if opt.model_s in [
                'MobileNetV2', 'ShuffleV1', 'ShuffleV2'
            ] else 0.1
        else:
            opt.learning_rate = 0.01 if opt.model_s in [
                'MobileNetV2', 'ShuffleV1', 'ShuffleV2'
            ] else 0.05
    ...
    if opt.distill == 'gkdfd':
        ...
        if opt.beta is None:
            opt.beta = 1.2
        if opt.batch_size % opt.gkdfd_classes_per_batch != 0:
            raise ValueError(
                'GK-DFD requires batch_size divisible by '
                'gkdfd_classes_per_batch'
            )
        if not 0.0 <= opt.gkdfd_cac_scale <= 1.0:
            raise ValueError('gkdfd_cac_scale must be in [0, 1]')
        if opt.gkdfd_pca_ratio < 1.0:
...
def main():
    ...
    if opt.dataset == 'cifar100':
        if opt.distill == 'gkdfd':
            train_loader, val_loader, n_data = get_cifar100_dataloaders_balanced(
                batch_size=opt.batch_size,
                num_workers=opt.num_workers,
                classes_per_batch=opt.gkdfd_classes_per_batch,
            )
        elif opt.distill in ['crd', 'hkd']:
    ...
    if opt.distill == 'kd':
        ...
        if GNNLoss is None:
            raise ImportError(
                'HKD requires a compatible DGL installation'
    ...
    elif opt.distill == 'gkdfd':
        opt.s_dim = feat_s[-1].shape[1]
        opt.t_dim = feat_t[-1].shape[1]
        criterion_kd = GKDFDLoss(opt)
