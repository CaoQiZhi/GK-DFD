...
class CIFAR100InstanceSample(datasets.CIFAR100):
    ...
    def __init__(self, root, train=True,
        ...
        # cls_positive 相同 class 的所有 item
        # cls_negative class 不等于 i 的所有 item
...
class ClassBalancedBatchSampler(Sampler):
    """Sample C classes and K examples per class for every batch."""

    def __init__(self, targets, batch_size, classes_per_batch):
        if batch_size % classes_per_batch != 0:
            raise ValueError(
                'batch_size must be divisible by classes_per_batch'
            )
        ...
        if classes_per_batch > len(self.classes):
            raise ValueError(
                'classes_per_batch exceeds the number of dataset classes'
            )
        self.class_indices = {
    ...
    def __iter__(self):
        for _ in range(self.num_batches):
            ...
            for class_id in selected_classes:
                ...
                replace = self.samples_per_class > len(candidates)
                selected = np.random.choice(
                    candidates, self.samples_per_class, replace=replace
                )
...
def get_cifar100_dataloaders_balanced(batch_size=64, num_workers=8,
                                      classes_per_batch=8):
    """CIFAR-100 loaders with the class-wise sampling used by GK-DFD."""
    data_folder = get_data_folder()
