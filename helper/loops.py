...
def train_distill(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        if opt.distill == 'kd':
            ...
            f_s = feat_s[-1]
            f_t = feat_t[-1]
            loss_graph, loss_feature = criterion_kd(
                f_s, logit_s, f_t, logit_t, target
            )
            loss_kd = (
        ...
        if idx % opt.print_freq == 0:
            ...
            if opt.distill == 'gkdfd':
                print(
                    'GK-DFD Graph {0.val:.4f} ({0.avg:.4f})\t'
                    'Feature {1.val:.4f} ({1.avg:.4f})'.format(
                        losses_graph, losses_feature
                    )
                )
            sys.stdout.flush()
